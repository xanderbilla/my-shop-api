...
public class AdminSecurityService {
    ...
    public AdminSecurityService(JwtTokenService jwtTokenService, UserService userService) {
    ...
    public String getCurrentAdminId() {
        try {
            ...
            if (accessToken != null && jwtTokenService.isValidToken(accessToken)) {
                String adminEmail = jwtTokenService.extractUsernameFromToken(accessToken);

                // Get the admin user details to extract the name
                try {
                    User adminUser = userService.getUserByEmail(adminEmail);
                    return adminUser != null ? adminUser.getCustName() : adminEmail;
                } catch (Exception e) {
                    System.err.println("Failed to get admin name, using email: " + e.getMessage());
                    return adminEmail; // fallback to email if name lookup fails
                }
            }
        } catch (Exception e) {
