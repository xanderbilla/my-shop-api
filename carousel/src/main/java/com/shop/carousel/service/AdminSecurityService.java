...
import com.shop.carousel.exception.UserNotAuthenticatedException;
import com.shop.carousel.exception.AdminAccessDeniedException;
...
/**
 * Custom Security Service for @PreAuthorize expressions in Carousel Service
 * 
...
public class AdminSecurityService {
    ...
    public boolean isAdmin() {
        try {
            ...
            if (request == null) {
                System.err.println("CAROUSEL SECURITY ALERT: No HTTP request context available");
            ...
            if (accessToken == null) {
                System.err.println("CAROUSEL SECURITY ALERT: Access denied - No access token found in cookies");
                throw new UserNotAuthenticatedException("Please login first to access this resource.");
            }
            ...
            if (!jwtTokenService.isValidToken(accessToken)) {
                System.err.println(
                        "CAROUSEL SECURITY ALERT: Access denied - Invalid or expired token (failed JWKS signature verification)");
                throw new UserNotAuthenticatedException("Please login first to access this resource.");
            }
            ...
            if (!userGroups.contains("ADMIN")) {
                try {
                    String username = jwtTokenService.extractUsernameFromToken(accessToken);
                    System.err.println("CAROUSEL SECURITY ALERT: Access denied - User '" + username
                            + "' attempted admin access with groups " + userGroups + " (ADMIN group required)");
                } catch (Exception e) {
                    System.err.println("CAROUSEL SECURITY ALERT: Access denied - User has groups " + userGroups
            ...
            try {
                ...
                System.out.println("CAROUSEL ADMIN ACCESS GRANTED: User '" + adminUsername
                        + "' authenticated successfully with ADMIN group");
            } catch (Exception e) {
                System.out.println("CAROUSEL ADMIN ACCESS GRANTED: Admin user authenticated successfully");
            }
            ...
            System.err.println("CAROUSEL SECURITY ERROR: Exception during admin validation: " + e.getMessage());
            throw new UserNotAuthenticatedException("Please login first to access this resource.");
        }
    }
    ...
    public boolean validateAdminAccess() {
        try {
            ...
            System.out.println("CAROUSEL ADMIN ACCESS VALIDATION: User '" + username +
                    "', Groups: " + userGroups +
                    ", Access Granted: " + hasAdminAccess);

            return hasAdminAccess;
        } catch (Exception e) {
            System.err.println("Carousel admin access validation error: " + e.getMessage());
            return false;
        }
    }
