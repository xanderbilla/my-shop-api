...
/**
 * Custom Security Service for @PreAuthorize expressions in Categories Service
 * 
...
public class AdminSecurityService {
        ...
        try {
            ...
            if (userGroups == null || userGroups.isEmpty()) {
                try {
                    ...
                    System.err.println(
                            "CATEGORIES SECURITY ALERT: Access denied - User '" + username + "' has no Cognito groups assigned");
                } catch (Exception e) {
            ...
            if (!userGroups.contains("ADMIN")) {
                try {
                    String username = jwtTokenService.extractUsernameFromToken(accessToken);
                    System.err.println("CATEGORIES SECURITY ALERT: Access denied - User '" + username
            ...
            try {
                ...
                System.out.println("CATEGORIES ADMIN ACCESS GRANTED: User '" + adminUsername
                        + "' authenticated successfully with ADMIN group");
            } catch (Exception e) {
                System.out.println("CATEGORIES ADMIN ACCESS GRANTED: Admin user authenticated successfully");
            }
    ...
    public boolean validateAdminAccess() {
        try {
            ...
            System.out.println("CATEGORIES ADMIN ACCESS VALIDATION: User '" + username +
                    "', Groups: " + userGroups +
                    ", Access Granted: " + hasAdminAccess);

            return hasAdminAccess;
        } catch (Exception e) {
            System.err.println("Categories admin access validation error: " + e.getMessage());
            return false;
        }
    }
